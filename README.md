...
## 표준 출력
Go 언어에서 표준 출력은 fmt 패키지의 Println, Printf, Print 함수를 사용합니다.

- `Println` 함수는 괄호 안에 적힌 값을 출력한 뒤 줄바꿈 문자를 추가합니다.
- `Printf` 함수는 서식 지정 문자열(format string)을 사용해 값을 출력합니다. 서식 지정 문자열은 출력 형식을 지정하는 특수 문자로 구성된 문자열입니다. 예를 들어 %d는 정수형 값을, %f는 실수형 값을 의미합니다.
- `Print` 함수는 괄호 안에 적힌 값을 출력하지만 줄바꿈 문자를 추가하지 않습니다.
...
## 표준 입력
Go 언어에서 표준 입력은 fmt 패키지의 Scanln, Scanf, Scan 함수를 사용합니다.
- `Scanln` 함수는 공백 문자(스페이스, 탭, 개행 문자 등)를 기준으로 입력 값을 구분하여 변수에 저장합니다. 개행 문자를 만나면 입력을 종료합니다.
- `Scanf` 함수는 서식 지정 문자열(format string)을 사용해 입력 값을 변수에 저장
- `Scan` 함수는 공백 문자를 기준으로 입력 값을 구분하여 변수에 저장합니다. 개행 문자를 만나도 입력을 종료하지 않습니다.

# 함수
![image.png](./imgs/function.png)
함수는 ① 함수 키워드, ② 함수명, ③ 매개변수, ④ 반환 타입, ⑤ 함수 코드 블록으로 구성됩니다.
① func 키워드를 사용해서 함수 정의를 알립니다.
② 그 뒤에 함수명이 옵니다. 함수명의 명명 규칙은 변수명과 같습니다. 첫 글자가 대문자인 함수는 패키지 외부로 공개되는 함수입니다
③ 소괄호 안에 매개변수를 넣습니다. 매개변수는 함수 코드 수행 시 필요한 입력값입니다. 매개변수가 필요하지 않으면 비워둡니다. 
④ 반환 타입이 옵니다. 반환하는 값이 있으면 적고, 아니면 비워둡니다.
